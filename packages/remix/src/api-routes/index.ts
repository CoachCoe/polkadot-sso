...
import { RemixAuthConfig, RemixAuthHandler } from '../types';

export function createAuthApiRoutes(config: RemixAuthConfig): RemixAuthHandler {
...
Nonce: ${nonce}`;
    ...
    async verify(request: Request): Promise<Response> {
      try {
        ...
        response.headers.set(
          'Set-Cookie',
          `polkadot-auth=${sessionId}; HttpOnly; Path=/; Max-Age=${config.sessionMaxAge || 3600}; SameSite=Lax`
        );
    ...
    async signOut(request: Request): Promise<Response> {
      try {
        ...
        response.headers.set(
          'Set-Cookie',
          'polkadot-auth=; HttpOnly; Path=/; Max-Age=0; SameSite=Lax'
        );
