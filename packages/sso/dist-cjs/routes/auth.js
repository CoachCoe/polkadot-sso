...
function verifySignature(message, signature, address) {
    try {
        ...
        const logger = (0, logger_1.createLogger)('signature-verification');
...
const createAuthRouter = (tokenService, challengeService, auditService, clients) => {
    ...
    const loginHandler = async (req, res) => {
        try {
            ...
            const validation = (0, validation_2.validateAuthRequest)(req);
    ...
    const challengeHandler = async (req, res) => {
      ...
      <html lang="en">
        <head>
          <meta charset="utf-8">
          <meta name="viewport" content="width=device-width, initial-scale=1">
          <title>${(0, sanitization_1.escapeHtml)(client.name)} - Sign Message</title>
          <link rel="stylesheet" href="/styles/main.css">
          <link rel="stylesheet" href="/styles/home.css">
          <script nonce="${res.locals.nonce}">
            window.CHALLENGE_DATA = {
              address: "${(0, sanitization_1.escapeHtml)(String(address ?? ''))}",
              ...
              <div class="hero-content">
                ...
                <div class="container" style="max-width: 600px; margin-top: 40px; padding-bottom: 40px;">
                  <div class="message-box" style="background: #f8fafc; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                    <p style="margin-bottom: 10px;"><strong>Message:</strong> ${(0, sanitization_1.escapeHtml)(challenge.message)}</p>
                    <p><strong>Address:</strong> ${(0, sanitization_1.escapeHtml)(String(address ?? ''))}</p>
                  </div>
    ...
    const verifyHandler = async (req, res) => {
        try {
            ...
            const logger = (0, logger_1.createLogger)('auth-verification');
            ...
            const authCode = Array.from((0, crypto_1.randomBytes)(32))
    ...
    const tokenHandler = async (req, res) => {
        try {
            ...
            const client = await (0, validation_2.validateClientCredentials)(req);
            if (!client) {
                return res.status(401).send('Invalid client credentials');
            }
            // TODO: Create an AuthCodeService to handle this operation
            // For now, we'll use a simple approach - this needs to be implemented properly
            const typedAuthCode = {
                address: 'temp-address', // This should be retrieved from the auth code
                expires_at: Date.now() + 5 * 60 * 1000,
            };
    ...
    router.get('/login', rateLimiters.login, (0, validation_1.sanitizeRequest)(), (0, validation_1.validateBody)(loginSchema), loginHandler);
    /**
     * @swagger
     * /auth/challenge:
     *   get:
    ...
    (0, validation_1.sanitizeRequest)(), (0, validation_1.validateBody)(challengeSchema), challengeHandler);
    /**
     * @swagger
     * /auth/verify:
     *   post:
     ...
     *         description: Internal server error
     */
    router.post('/verify', rateLimiters.verify, (0, validation_1.sanitizeRequest)(), (0, validation_1.validateBody)(verifySchema), verifyHandler);
    /**
     * @swagger
     * /auth/signout:
     *   post:
    ...
    router.post('/signout', async (req, res) => {
        try {
            ...
            const logger = (0, logger_1.createLogger)('auth-signout');
