...
const telegram_1 = require("@polkadot-auth/telegram");
 ...
 */
function getTelegramProvider() {
    if (!telegramProvider) {
        telegramProvider = (0, telegram_1.createTelegramProvider)({
...
router.post('/verify', async (req, res) => {
    try {
        ...
        const success = await telegramProvider.completeAuthentication(challengeId, userId, walletAddress, signature);
        if (!success) {
            return res.status(400).json({
        ...
        switch (action) {
            case 'approve':
                ...
                if (data?.walletAddress && data?.signature) {
                    await telegramProvider.completeAuthentication(challengeId, userId, data.walletAddress, data.signature);
                }
                break;
            case 'reject':
