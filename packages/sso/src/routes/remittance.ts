...
// Initialize services
const authService = new RemittanceAuthService();
const remittanceService = new RemittanceService(authService);
const complianceService = new ComplianceService();

// Middleware to ensure user is authenticated
const requireAuth = (req: Request, res: Response, next: Function) => {
  const session = req.session as unknown as RemittanceSession;
...
const requireCustodyLevel = (minLevel: number) => {
...
router.post('/send', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const transaction = await remittanceService.createRemittanceTransaction(
      session.userId,
      recipient,
      amount,
      targetCurrency
    ...
    res.status(500).json({
      error: (error as any).message || 'Failed to send money',
...
router.get('/history', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      error: (error as any).message || 'Failed to get transaction history',
      code: 'HISTORY_FAILED',
    });
  }
});
 ...
 */
router.get('/transaction/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const session = req.session as unknown as RemittanceSession;
    ...
    res.status(500).json({
      error: (error as any).message || 'Failed to get transaction',
 ...
 */
router.post('/upgrade-custody', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const success = await authService.upgradeCustodyLevel(
      session.userId,
      session.custodyLevel,
      targetLevel,
      additionalAuth
    );
    ...
    res.status(500).json({
      error: (error as any).message || 'Failed to upgrade custody level',
      code: 'UPGRADE_FAILED',
    });
  }
});
...
router.post('/claim', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      error: (error as any).message || 'Failed to claim remittance',
      code: 'CLAIM_FAILED',
    });
  }
});
 ...
 */
router.get('/custody-levels', requireAuth, async (req: Request, res: Response) => {
  try {
    const session = req.session as unknown as RemittanceSession;
    ...
    res.status(500).json({
      error: (error as any).message || 'Failed to get custody levels',
 ...
 */
router.post('/kyc', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      error: (error as any).message || 'KYC verification failed',
      code: 'KYC_FAILED',
    });
  }
});
...
router.get('/compliance', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      error: (error as any).message || 'Failed to get compliance status',
 ...
 */
router.get('/quote', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      quote: {
        ...
        recipientAmount,
        fees,
        totalCost: amountNum + (fees as any).total,
    ...
    res.status(500).json({
      error: (error as any).message || 'Failed to get quote',
      code: 'QUOTE_FAILED',
    });
  }
});
