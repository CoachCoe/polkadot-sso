...
exports.configManager = exports.ProductionConfigManager = void 0;
...
class ProductionConfigManager {
    ...
    loadExchangeRateConfig() {
        return {
            provider: process.env.EXCHANGE_RATE_PROVIDER || 'coingecko',
    ...
    validateConfig() {
        ...
        if (this.config.remittance.enabled && !this.config.remittance.treasuryAddress) {
            this.validationErrors.push('REMITANCE_TREASURY_ADDRESS is required when remittance is enabled');
        }
        // Validate chain config
        if (!this.config.chains.rpcUrls[this.config.chains.defaultChain]) {
            this.validationErrors.push(`RPC URL not configured for default chain: ${this.config.chains.defaultChain}`);
        }
     ...
     */
    parseArrayEnv(key, defaultValue) {
        const value = process.env[key];
        if (!value)
            return defaultValue;
        return value
 ...
 */
exports.configManager = new ProductionConfigManager();
/**
...
function validateConfig() {
    exports.configManager.loadConfig();
    if (!exports.configManager.isValid()) {
        throw new Error(`Configuration validation failed:\n${exports.configManager.getValidationErrors().join('\n')}`);
    }
}
